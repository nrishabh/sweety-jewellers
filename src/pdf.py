...
def drawNewSection(c, group, category, y_pos):
    ...
    c.drawCentredString((0.5*cm)+((20*cm)/2),841.89-(y+((0.9*cm)/2)), group+": "+category)
...
def create_pdf(raw_DB, price_col, OUT_FILE, objProgressBar, ROOT):
    ...
    c = canvas.Canvas(filename=OUT_FILE+".pdf",
    ...
    for item in DB.index:
        ...
        if (DB.at[item, "Group"]!=curr_group) or (DB.at[item, "Category"]!=curr_category):
            if x_pos!=1:
                x_pos, y_pos = incr_y(c, x_pos, y_pos)
